...
final class CoreDataMigrator {
    ...
    func migrateStore(at storeURL: URL, toVersion version: CoreDataMigrationVersion) throws {
        ...
        for migrationStep in migrationSteps {
            ...
            do {
                ...
                let description = "Failed attempting to migrate from \(migrationStep.sourceModel) " +
                    "to \(migrationStep.destinationModel), error: \(error)"
                assertionFailure(description)
                throw DatabaseError(code: .failiedToMigrate, description: description)
            }
    ...
    private func migrationStepsForStore(
        ...
        guard
            ...
            let description = "Unknown store version at URL \(storeURL)"
            assertionFailure(description)
            throw DatabaseError(code: .failiedToMigrate, description: description)
        }
    ...
    private func migrationSteps(
        ...
        while sourceVersion != destinationVersion, let nextVersion = sourceVersion.nextVersion() {
            guard
                ...
                throw DatabaseError(code: .failiedToMigrate, description: "Unable to determine migration step")
            }
    ...
    /// This method allows to force migrate sqlite-wal intermediate storage
    /// Description: https://www.sqlite.org/wal.html
    private func forceWALCheckpointingForStore(at storeURL: URL) throws {
        ...
        do {
            ...
            let desrciption = "Unable to transfer WAL \n Error: \(error.localizedDescription)"
            assertionFailure(desrciption)
            throw DatabaseError(code: .failiedToMigrate, description: desrciption)
        }
    }
}
    ...
    static func compatibleVersionForStoreMetadata(_ metadata: [String: Any]) -> CoreDataMigrationVersion? {
        let compatibleVersion = CoreDataMigrationVersion.allCases.first {
            let model = NSManagedObjectModel.managedObjectModel(
                forResource: $0.modelName,
                inModel: CoreDataConst.modelName
            )
