...
final class DatabaseCatalogSource: CatalogSource {
    ...
    let itemsPublisher: AnyPublisher<[CatalogItem], AppError>
    ...
    func read() -> AnyPublisher<IdentifiedArrayOf<CatalogItem>, AppError> {
        fetchOrGetCurrentItems()
            .map { items in
                ...
                return IdentifiedArrayOf<CatalogItem>(uniqueElements: mappedItems)
            }
            .eraseToAnyPublisher()
    }
        ...
        fetchOrGetCurrentItems()
            .flatMap { items in
                Future { [weak self] promise in
                    self?.databaseService.writeAsync { context in
                        var items = items
                        guard let initialIndex = items.index(id: item.id) else { return }

                        items.move(fromOffsets: .init(integer: initialIndex), toOffset: index)

                        try context.updateIndices(items: &items, offset: min(initialIndex, index))
                        try context.save()
                    } completion: {
                        Self.completion(promise: promise, result: $0)
                    }
...
private extension Database.CatalogItem {
    ...
    func convertToModel() -> CatalogItem {
